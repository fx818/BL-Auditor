"""Title vs Specs Agent 2 — async LangChain wrapper.

Reads the buylead's item title and ISQ specs and asks the LLM to classify
whether the title is consistent with the stated specs.
...
log = logging.getLogger("bl-auditor.title_vs_specs_agent2")
_LLM_MAX_RETRIES = int(os.getenv("LLM_MAX_RETRIES", "2"))


class TitleVsSpecsAgent2State(TypedDict, total=False):
    offer_id: str
...
def _read_prompt() -> str:
    ...
    return get_active_prompt("title_vs_specs_agent2")[0]
...
async def _prepare_input(state: TitleVsSpecsAgent2State) -> TitleVsSpecsAgent2State:
...
async def _classify(state: TitleVsSpecsAgent2State) -> TitleVsSpecsAgent2State:
    ...
    if not agent_input.get("title", "").strip() and agent_input.get("isq_count", 0) == 0:
        result = {
            "Display_id": agent_input.get("Display_id", state.get("offer_id", "")),
            "Status": "Not Available",
            "Score": 0.0,
            "Confidence": "High",
            "Reason": "No title or ISQ specs were provided in the BuyLead.",
            "Issues": [{"type": "missing", "fields": ["title", "isq_table"], "detail": "Both title and ISQs are empty."}],
        }
        return {
            "system_prompt": "(skipped — no title and no ISQs)",
            "user_message": "(skipped — no title and no ISQs)",
    ...
    user_message = (
        f"Audit the following BuyLead's title against its ISQ specs.\n\n"
        f"Title: {agent_input['title'] or '(empty)'}\n\n"
    ...
    for attempt in range(1, _LLM_MAX_RETRIES + 2):
        try:
            ...
            last_exc = exc
            log.warning("title_vs_specs_agent2 attempt %d/%d failed: %s", attempt, _LLM_MAX_RETRIES + 1, exc)
            if attempt <= _LLM_MAX_RETRIES:
                await asyncio.sleep(2 ** (attempt - 1))
    else:
        raise RuntimeError(f"title_vs_specs_agent2 exhausted {_LLM_MAX_RETRIES + 1} attempts: {last_exc}") from last_exc

    raw_output = str(response.content)
    try:
        parsed = _clean_output(raw_output)
    except (ValueError, json.JSONDecodeError) as exc:
        raise RuntimeError(f"title_vs_specs_agent2 parse failed: {exc}. Raw: {raw_output[:500]!r}") from exc
...
def _build_graph():
    graph = StateGraph(TitleVsSpecsAgent2State)
...
async def run_title_vs_specs_agent2(
    offer_id: str,
    buylead_response: Dict[str, Any],
    _trace: bool = False,
) -> Dict[str, Any]:
    ...
    _cfg = {"callbacks": [_h], "run_name": f"title_vs_specs_agent2:{offer_id}"} if _h else {}

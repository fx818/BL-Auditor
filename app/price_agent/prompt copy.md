...
- Price Match     : {{ price_match }}
    "exact"   = Median_Price and Q3_Price came from a real mcat_data row for this (Category, Unit) — USE THEM
...
CRITICAL — DO NOT CONFUSE THESE TWO SIGNALS:
- `price_match` controls the VERDICT. It is the ONLY flag that can trigger "Unverifiable".
- `evidence_match` controls CONFIDENCE only. It can NEVER trigger "Unverifiable".
- If `Median_Price > 0`, you HAVE a usable anchor — proceed with Steps 4–7 even if `evidence_match` is "no_data" or `evidence_count` is 0.
- Never report `Unit_Price_Used = 0` or `Price_Value_By_AI = "₹ 0 - ₹ 0"` when `Median_Price > 0` was given to you.
...
DATA-PROVENANCE OVERRIDES (applied AFTER Steps 4–7):

VERDICT OVERRIDES — only `price_match` and the numeric Median_Price control this:
  - If Median_Price == 0 (i.e. price_match == "no_data") → set Price_Results = "Unverifiable",
    Confidence = "Low". Explain in Price_Reason that no reference price exists for this category-unit.
  - Otherwise (Median_Price > 0) → DO NOT mark Unverifiable for any data-provenance reason.
    Even if evidence_match == "no_data" and evidence_count == 0, keep your Step-7 verdict.

CONFIDENCE-ONLY OVERRIDES — these never affect the verdict:
  - If Evidence Match == "no_data" or "unit_only" → cap Confidence at "Medium".
  - If Evidence Count < 3 → reduce Confidence by one level (Medium → Low; High → Medium).

WORKED EXAMPLE (so the contract is unambiguous):
  Input: Qty="1 Piece", Order_Value="Rs. Upto 1,000", median=999, q3=3575,
         price_match="exact", evidence_match="no_data", evidence_count=0, BL_card=[]
  Correct behavior:
    BL absent → Unit_Price_Used = Median_Price = 999
    Base_Value = 1 × 999 = 999, tolerance ±20% (BL absent)
    Price_Value_By_AI = "₹ 799 - ₹ 1199"
    Buyer range = ₹ 0 – ₹ 1000 → overlap exists → Price_Results = "Correct"
    Confidence: capped at "Medium" because evidence_match=no_data; one step down
                because evidence_count<3 → final Confidence = "Low"
  Wrong behavior (do NOT do this):
    Marking Unverifiable, setting Unit_Price_Used=0, Price_Value_By_AI="₹ 0 - ₹ 0"
    just because evidence_match=no_data.

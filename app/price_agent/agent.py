"""Price Agent — async LangChain wrapper.

Reads the buylead's MCAT, title, quantity, ISQs, and price, and asks the LLM
to evaluate whether the stated price is plausible for the category.
...
LLM output contract (from prompt.md):
    {
        ...
        "isq_tier": str,
        "price_signals": [...],
        "fair_unit_price_range": str,
        "ai_order_value_range": str,
        "stored_order_value_range": str,
        "implied_unit_price_range": str,
...
log = logging.getLogger("bl-auditor.price_agent")
...
def _read_prompt() -> str:
    ...
    return get_active_prompt("price")[0]
...
def _bl_detail_and_keys(source: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        log.warning("price_agent ISQ parse failed (qty/unit will be empty): %s", exc)
...
async def _prepare_input(state: PriceState) -> PriceState:
    ...
    bl_data = state["buylead_response"].get("RESPONSE", {}).get("DATA", {}) or {}
    isq_list = parse_enrichmentinfo_to_isq(bl_data.get("ENRICHMENTINFO"))
...
async def _price_classify(state: PriceState) -> PriceState:
    ...
    user_text = "\n".join([
        ...
        f"ISQ: {json.dumps(agent_input.get('ISQ', []), ensure_ascii=False)}",
        f"BL_card: {json.dumps(agent_input.get('BL_card', []), ensure_ascii=False)}",
        f"q1: {agent_input.get('q1', '')}",
    ...
    for attempt in range(1, _LLM_MAX_RETRIES + 2):
        try:
            ...
            last_exc = exc
            log.warning(
                "price_agent LLM attempt %d/%d failed: %s: %s",
        ...
        raise RuntimeError(
            f"Price LLM exhausted {_LLM_MAX_RETRIES + 1} attempts: {last_exc}"
        ) from last_exc
...
async def run_price_agent(
    ...
    _cfg = {"callbacks": [_h], "run_name": f"price_agent:{offer_id}"} if _h else {}
